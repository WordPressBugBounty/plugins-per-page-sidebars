...
/*	Copyright 2018 Brian Layman ( email : plugins@ehermitsinc.com )
...
function pps_inner_custom_box( $post ) {
	...
	foreach ( $wp_registered_sidebars as $id => $sidebar ) {
		...
		if ( substr( $name, 0, strlen( SIDEBAR_PREFIX ) ) != SIDEBAR_PREFIX ) {
			echo '			<span class="tcc-pps-radio-option"><input type="radio" name="tcc-pps-replace" value="' . esc_attr( $id ) .
			'" class="tcc-pps-radio" ' . tcc_pps_add_checked( $replaced_sidebar, $id ) . '/> ' . esc_html( $name ) . ' - ' . esc_html( $description ) . '</span><br/>';
		}
	}
	echo '		</div>';
}
...
function pps_reg_all_custom_sidebars() {
	...
	foreach ( $posts as $post_id ) {
		...
		if (  '' === $post_name ) {
			$post_name = sanitize_title( $post->post_title, '', 'save' );
		}
